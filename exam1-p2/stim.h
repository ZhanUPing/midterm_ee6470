...
SC_MODULE(stim) {
  ...
  sc_in_clk i_clk;
  sc_out<bool> o_rst;
  sc_fifo_out<sc_ufixed_fast<WIDTH> > o_a;
  sc_fifo_out<sc_ufixed_fast<WIDTH> > o_b;
  sc_fifo_in<sc_ufixed_fast<WIDTH+1> > i_sum;
  //Store the previous inputs to FIFOs
  sc_ufixed_fast<WIDTH> t_a;
  sc_ufixed_fast<WIDTH> t_b;
  sc_ufixed_fast<WIDTH+1> t_sum;
